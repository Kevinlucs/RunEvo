...
| 9 | IA indisponível → `blueprint.source === 'local'` | `f09` | Não é input-dependente: **todas** as fixtures passam pelo caminho local (harness rejeita `fetch` de propósito). `f09` é a referência designada. **Achado (pós-Grupo D):** o golden guarda `blueprint.source === "fallback: <mensagem de erro do fetch>"`, não um enum limpo `'local'` — `generateBlueprint` (ai-coach.js:855-867) passa `error.message` como `reason` para `buildFallbackBlueprint`. O critério `=== 'local'` não bate literalmente com nenhum valor real do legado; ativar o teste como `blueprint.source.startsWith('fallback')`, salvo instrução em contrário |
...
**0 divergências.** Cenários 9/10 continuam `test.todo` (comportamento de
pipeline/fingerprint, não input de fixture — ver linha 9 acima para o achado
sobre `blueprint.source`).
