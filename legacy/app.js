const RunEvo_BUILD_VERSION = 'v154-single-export-paywall-cta';
...
function shouldHideBottomNav(page) {
  return ['settings', 'shoes', 'shoe-form', 'general-settings', 'privacy', 'support', 'runevo-plus', 'my-resources', 'workout', 'phase-detail', 'plan-preview'].includes(page);
}

function updateBottomNavState(page) {
  ...
  if (bottomNav) {
    bottomNav.classList.toggle('hidden', shouldHideBottomNav(page));
  }
}


function getRunEvoSubscriptionPlan() {
  try {
    const profile = typeof UserProfileService !== 'undefined' ? UserProfileService.getCurrentProfile?.() : null;
    const localProfile = typeof StorageService !== 'undefined' ? StorageService.loadUserProfile?.() : {};
    const raw = typeof StorageService !== 'undefined'
      ? StorageService.getRaw?.('runevo_subscription', '')
      : '';

    const value = String(
      localProfile?.plan ||
      localProfile?.subscription ||
      profile?.plan ||
      profile?.subscription ||
      profile?.localProfile?.plan ||
      profile?.localProfile?.subscription ||
      raw ||
      'free'
    ).toLowerCase();

    if (['plus', 'pro', 'premium', 'paid', 'runevo+'].some(flag => value.includes(flag))) {
      return 'plus';
    }
...
function isRunEvoPlusUser() {
  return getRunEvoSubscriptionPlan() === 'plus';
}

function getRunEvoPlanLabel() {
  return isRunEvoPlusUser() ? 'RunEvo+' : 'Free';
}

function getRunEvoPlanDetails() {
  const plus = isRunEvoPlusUser();
  return {
    key: plus ? 'plus' : 'free',
    label: plus ? 'RunEvo+' : 'Free',
    headline: plus ? 'Controle total da evolução' : 'Essencial para começar',
    description: plus
      ? 'Múltiplas planilhas, histórico, comparação técnica, Excel profissional e auditoria avançada.'
      : 'Uma planilha ativa, treinos, check-ins, estatísticas básicas e PDF profissional.',
    activePlanLimit: plus ? 'Ilimitado com histórico' : '1 planilha ativa',
    exports: plus ? 'PDF + Excel profissional' : 'PDF profissional',
    comparison: plus ? 'Comparação liberada' : 'Bloqueada'
  };
}


function getRunEvoSubscriptionState() {
  const fallback = {
    plan: getRunEvoSubscriptionPlan(),
    billing: null,
    status: getRunEvoSubscriptionPlan() === 'plus' ? 'active' : 'inactive',
    startedAt: null,
    expiresAt: null,
    source: 'local'
  };
  ...
  try {
    const saved = StorageService.getJSON?.('runevo_subscription_state', null);
    if (!saved || typeof saved !== 'object') return fallback;

    return {
      ...fallback,
      ...saved,
      plan: String(saved.plan || fallback.plan).toLowerCase().includes('plus') ? 'plus' : 'free'
    };
  } catch (_) {
    return fallback;
  }
...
function saveRunEvoSubscriptionState(nextState = {}) {
  const current = getRunEvoSubscriptionState();
  const normalized = {
    ...current,
    ...nextState,
    plan: String(nextState.plan || current.plan || 'free').toLowerCase().includes('plus') ? 'plus' : 'free',
    updatedAt: new Date().toISOString()
  };

  try {
    StorageService.setJSON?.('runevo_subscription_state', normalized);
  } catch (_) {}

  return normalized;
}

function getRunEvoBillingLabel() {
  const state = getRunEvoSubscriptionState();
  if (state.plan !== 'plus') return 'Free';
  if (state.billing === 'annual') return 'Anual';
  if (state.billing === 'monthly') return 'Mensal';
  return 'RunEvo+';
}

function getRunEvoUpgradeTriggerLabel(featureKey = '') {
  const labels = {
    planComparison: 'comparar planilhas',
    planHistory: 'guardar histórico',
    excelExport: 'exportar Excel profissional',
    advancedQuality: 'ver auditoria avançada'
  };
  return labels[featureKey] || 'usar recursos Pro';
}

function setRunEvoSubscriptionPlan(plan = 'free') {
  const normalized = String(plan || 'free').toLowerCase().includes('plus') ? 'plus' : 'free';
  const current = typeof StorageService !== 'undefined' ? StorageService.loadUserProfile?.() || {} : {};
  ...
  if (typeof StorageService !== 'undefined') {
    ...
    StorageService.setRaw?.('runevo_subscription', normalized);
    saveRunEvoSubscriptionState({
      plan: normalized,
      status: normalized === 'plus' ? 'active' : 'inactive',
      billing: normalized === 'plus' ? (getRunEvoSubscriptionState().billing || 'annual') : null
    });
  }

  updateRunEvoBrandLogo();
  renderSettingsPage?.();
  renderRunEvoPlusPage?.();
  return normalized;
}


function updateRunEvoPremiumUI() {
  const plus = isRunEvoPlusUser();
  document.querySelectorAll('[data-premium-feature]').forEach(el => {
    el.classList.toggle('is-premium-locked', !plus);
    el.classList.toggle('is-premium-unlocked', plus);
    el.setAttribute('aria-label', plus ? 'Recurso RunEvo+ liberado' : 'Recurso RunEvo+ bloqueado. Toque para conhecer o Plus.');
  });
}

function updateRunEvoBrandLogo() {
  const plus = isRunEvoPlusUser();
  const src = plus ? 'assets/img/logo-runevo-plus.png' : 'assets/img/logo-runevo.png';
  const alt = plus ? 'RunEvo+' : 'RunEvo';

  document.querySelectorAll('#app-header-logo, [data-runevo-brand-logo]').forEach(logo => {
    if (!logo) return;
    if (logo.getAttribute('src') !== src) logo.src = src;
    logo.alt = alt;
    logo.classList.toggle('is-plus', plus);
  });

  document.body.classList.toggle('is-runevo-plus', plus);
  document.body.classList.toggle('is-runevo-free', !plus);
  updateRunEvoPremiumUI?.();
}

window.RunEvoSubscriptionService = {
  getPlan: getRunEvoSubscriptionPlan,
  getState: getRunEvoSubscriptionState,
  saveState: saveRunEvoSubscriptionState,
  isPlus: isRunEvoPlusUser,
  setPlan: setRunEvoSubscriptionPlan,
  updateLogo: updateRunEvoBrandLogo
};

const RunEvo_FEATURES = {
  planComparison: {
    title: 'Comparação de planilhas',
    description: 'Compare score, risco, volume, longões e fases antes de trocar sua planilha ativa.'
  },
  planHistory: {
    title: 'Histórico de planilhas',
    description: 'Mantenha versões antigas, reative planos e acompanhe a evolução entre ciclos.'
  },
  excelExport: {
    title: 'Excel profissional',
    description: 'Exporte relatórios editáveis com semanas, treinos, progresso e métricas técnicas.'
  },
  advancedQuality: {
    title: 'Auditoria avançada do Motor Evo',
    description: 'Veja leituras profundas de progressão, risco, variedade e distribuição de carga.'
  }
};

const RunEvo_SALES_COMPARISON = [
  ['Planilha ativa', '1 plano por vez', 'Várias versões com histórico'],
  ['Nova planilha', 'Remove/substitui a atual', 'Compara antes de adotar'],
  ['Histórico', 'Não incluso', 'Ciclos antigos preservados'],
  ['Comparação', 'Não incluso', 'Score, risco, carga e longões'],
  ['Exportação', 'PDF profissional', 'PDF + Excel profissional'],
  ['Auditoria', 'Resumo essencial', 'Leitura avançada']
];

const RunEvo_FREE_RESOURCES = [
  ['🏁', '1 planilha ativa', 'Gere e adote uma planilha por vez.'],
  ['✅', 'Registro de treinos', 'Marque treinos concluídos ou pulados.'],
  ['🧠', 'Check-in semanal', 'Ajuste sua evolução com feedback de esforço.'],
  ['📊', 'Estatísticas básicas', 'Acompanhe km feitos, aderência e progresso.'],
  ['📄', 'PDF profissional', 'Exporte sua planilha para salvar ou compartilhar.'],
  ['👟', 'Controle de tênis', 'Registre seus tênis e acompanhe uso.']
];

const RunEvo_PLUS_RESOURCES = [
  ['🧬', 'Versões inteligentes', 'Gere mais de uma planilha sem apagar a atual.'],
  ['🗂️', 'Histórico completo', 'Guarde ciclos antigos e veja planilhas arquivadas.'],
  ['📊', 'Comparação técnica', 'Compare a planilha ativa com uma nova antes de adotar.'],
  ['📈', 'Excel profissional', 'Leve sua planilha para análise, impressão e backup.'],
  ['🛡️', 'Auditoria avançada', 'Veja se o plano está seguro, coerente e progressivo.'],
  ['✨', 'Identidade RunEvo+', 'Assinantes usam a experiência visual RunEvo+.']
];


function getRunEvoResourceCategoryLabel() {
  return isRunEvoPlusUser() ? 'Recursos Pro liberados' : 'Recursos Free liberados';
}

function getRunEvoResourceFooterCopy() {
  return isRunEvoPlusUser()
    ? 'Seu plano RunEvo+ está exibindo somente recursos Pro e recursos avançados disponíveis.'
    : 'Seu plano Free está exibindo somente recursos gratuitos. Os recursos Pro aparecem quando você assina o RunEvo+.';
}


const RunEvo_LOCKED_RESOURCE_CARDS = [
  ['planHistory', '🗂️', 'Histórico completo', 'Guarde ciclos antigos e acompanhe sua evolução entre planilhas.'],
  ['planComparison', '📊', 'Comparação técnica', 'Compare score, risco, volume, pico e longões antes de adotar.'],
  ['excelExport', '📈', 'Excel profissional', 'Exporte relatórios editáveis com semanas, treinos e métricas.'],
  ['pdfExport', '📄', 'PDF profissional', 'Gere uma versão visual organizada para imprimir, salvar ou compartilhar.'],
  ['advancedQuality', '🛡️', 'Auditoria avançada', 'Veja leituras técnicas mais profundas do Motor Evo.']
];

function renderRunEvoLockedResourceCard(featureKey, icon, title, text) {
  return `
    <button type="button"
      class="runevo-locked-resource-card premium-feature-card no-inner-cta"
      data-premium-feature="${escapeHTML(featureKey)}"
      onclick="showRunEvoPlusModal('${escapeHTML(featureKey)}')">
      <span class="locked-card-icon">${escapeHTML(icon)}</span>
      <span class="locked-card-copy">
...
function renderRunEvoLockedResources() {
  return RunEvo_LOCKED_RESOURCE_CARDS
    .map(([key, icon, title, text]) => renderRunEvoLockedResourceCard(key, icon, title, text))
    .join('');
}


function getRunEvoVisibleResources() {
  return isRunEvoPlusUser() ? RunEvo_PLUS_RESOURCES : RunEvo_FREE_RESOURCES;
}

function isFeatureUnlocked(featureKey) {
  if (!featureKey) return true;
  if (featureKey === 'basicPlan' || featureKey === 'pdfExport') return true;
  return isRunEvoPlusUser();
}

function showRunEvoPlusModal(featureKey = 'planComparison') {
  restoreRunEvoModalDefaultButtons?.();
  const feature = RunEvo_FEATURES[featureKey] || RunEvo_FEATURES.planComparison;
  ...
  if (!modal || !iconEl || !titleEl || !messageEl || !confirmBtn) {
    alert(`RunEvo+\n\n${feature.description}`);
    return;
  }

  iconEl.textContent = '⚡';
  titleEl.textContent = `${feature.title} é RunEvo+`;
  const trigger = getRunEvoUpgradeTriggerLabel(featureKey);
  messageEl.innerHTML = `
    <div class="plus-modal-content">
      <p>${escapeHTML(feature.description)}</p>
      <p class="plus-modal-highlight">Você tentou ${escapeHTML(trigger)}. Esse é um recurso Pro para atletas que querem comparar, preservar histórico e decidir com mais controle.</p>
      <div class="plus-benefit-list">
        <span>✅ Histórico completo de planilhas</span>
        <span>✅ Comparação entre versões</span>
        <span>✅ Excel profissional</span>
        <span>✅ Auditoria avançada do Motor Evo</span>
      </div>
      <p class="plus-modal-note">Pagamento real ainda será conectado. A estrutura mensal/anual já está preparada.</p>
    </div>
  `;
  ...
  confirmBtn.textContent = 'Assinar RunEvo+';
...
function getRunEvoPlanHistoryKey() {
  const user = typeof StorageService !== 'undefined' ? StorageService.getCurrentUser?.() || 'guest' : 'guest';
  return `${user}_runevo_plan_history`;
}

function loadRunEvoPlanHistory() {
  try {
    return StorageService.getJSON?.(getRunEvoPlanHistoryKey(), []) || [];
  } catch (_) {
    return [];
  }
}

function saveRunEvoPlanHistory(history = []) {
  try {
    return StorageService.setJSON?.(getRunEvoPlanHistoryKey(), Array.isArray(history) ? history : []);
  } catch (_) {
    return false;
  }
}

function archiveCurrentPlanForPlus(reason = 'substituída') {
  if (!isRunEvoPlusUser()) return false;

  const plan = typeof AICoach !== 'undefined' ? AICoach.loadPlan?.() : null;
  if (!plan) return false;

  const history = loadRunEvoPlanHistory();
  const item = {
    id: `plan-${Date.now()}`,
    archivedAt: new Date().toISOString(),
    reason,
    planName: plan.planName || plan.raceName || 'Planilha RunEvo',
    raceName: plan.raceName || '',
    totalWeeks: plan.totalWeeks || 0,
    score: plan.validation?.quality?.overall || null,
    risk: plan.validation?.summary?.riskLevel || null,
    distance: plan.validation?.summary?.raceDistanceKm || plan.targetDistanceKm || plan.distanceKm || null,
    peakWeekKm: plan.validation?.summary?.peakWeeklyKm || plan.validation?.summary?.peakWeekKm || null,
    longestTrainingRun: plan.validation?.summary?.biggestTrainingLongRunKm || plan.validation?.summary?.peakTrainingLongRunKm || null,
    plan
  };

  const itemSig = getPlanHistorySignature(plan);
  const filtered = history.filter(existing => getPlanHistorySignature(existing?.plan || existing) !== itemSig);
  filtered.unshift(item);
  return saveRunEvoPlanHistory(filtered.slice(0, 20));
}

function renderPlusLockBadge(label = 'RunEvo+') {
  return `<span class="plus-lock-badge">${escapeHTML(label)}</span>`;
}
...
function showPage(page) {
  ...
  if (!target) {
    console.warn(`RunEvo: página não encontrada: ${page}`);
    return false;
  }

  const activeElement = document.activeElement;
  if (activeElement && activeElement !== document.body && activeElement.closest?.('.page')) {
    activeElement.blur?.();
  }
  ...
  if (page === 'phases' && typeof renderPhases === 'function') renderPhases();
  if (page === 'stats' && typeof renderStats === 'function') renderStats();
  if (page === 'ai' && typeof renderAICoachPage === 'function') renderAICoachPage();
  if (page === 'settings' && typeof renderSettingsPage === 'function') renderSettingsPage();
  if (page === 'shoes' && typeof renderShoesPage === 'function') renderShoesPage();
  if (page === 'general-settings' && typeof renderGeneralSettingsPage === 'function') renderGeneralSettingsPage();
  if (page === 'support' && typeof renderSupportPage === 'function') renderSupportPage();
  if (page === 'runevo-plus' && typeof renderRunEvoPlusPage === 'function') renderRunEvoPlusPage();
  if (page === 'my-resources' && typeof renderMyResourcesPage === 'function') renderMyResourcesPage();
...
function showToast(message = '', type = 'info', duration = 3200) {
  ...
  let container = document.getElementById('runevo-toast-container');
  if (!container) {
    container = document.createElement('div');
    container.id = 'runevo-toast-container';
    container.className = 'runevo-toast-container';
    document.body.appendChild(container);
  }

  const toast = document.createElement('div');
  toast.className = `runevo-toast ${type || 'info'}`;
...
function renderEvolutionHistory() {
  ...
  if (!rows.length) {
    el.innerHTML = `
      <div class="evolution-empty">
        <strong>Nenhum planilha atual ainda.</strong>
...
function setExportStatus(message) {
  const el = document.getElementById('export-status') || document.getElementById('export-dados-status');
...
<body>
  <table>
    ...
    <tr>
      ...
      <td colspan="2" class="kpi-label">Maior longão de treino</td><td colspan="2" class="kpi-value">${excelKm(summary.biggestTrainingLongRun ?? summary.biggestLongRun)} km</td>
...
function showSimpleModal(icon = 'ℹ️', title = 'Aviso', message = '') {
  const modal = document.getElementById('modal-overlay');
  const iconEl = document.getElementById('modal-icon');
  const titleEl = document.getElementById('modal-title');
  const messageEl = document.getElementById('modal-message');
  ...
  messageEl.innerHTML = `<p>${escapeHTML(String(message || ''))}</p>`;
...
function clearExportCardSelection() {
  document.querySelectorAll('.export-action-card.selected').forEach(card => {
...
function handleExportCardOutsideClick(event) {
  if (!event.target.closest?.('.export-action-card')) {
    clearExportCardSelection();
  }
}

document.addEventListener('pointerdown', handleExportCardOutsideClick);



function requireRunEvoFeature(featureKey = '') {
  if (isFeatureUnlocked(featureKey)) return true;
  showRunEvoPlusModal(featureKey);
  return false;
}

function handleExportExcel() {
  if (!requireRunEvoFeature('excelExport')) {
    clearExportCardSelection();
    setExportStatus('Excel profissional disponível no RunEvo+. Toque no card para conhecer o Plus.');
    return;
  }
  ...
  setExportStatus(`Excel profissional exportado: ${filename}`);
}
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    th { background: #CCFF00; color: #111827; text-align: left; font-size: 11px; padding: 8px; font-weight: 900; }
...
function handleExportPDF() {
  if (!requireRunEvoFeature('pdfExport')) {
    clearExportCardSelection();
    setExportStatus('PDF profissional disponível no RunEvo+. Toque no card para conhecer o Plus.');
    return;
  }
  ...
  setExportStatus('PDF profissional aberto. Use “Salvar como PDF” na janela de impressão.');
}

function handleExportBackup() {
  showSimpleModal('☁️', 'Planilha vinculada à conta', 'A exportação em JSON foi removida. Sua planilha e evolução passam a ficar vinculadas ao perfil do atleta.');
}

function handleImportBackupClick() {
  showSimpleModal('☁️', 'Importação removida', 'A importação por JSON foi removida porque as planilhas passam a ficar vinculadas à conta do atleta.');
}
...
function setCheckboxIfPresent(id, checked) {
  const el = document.getElementById(id);
  if (!el) return;
  ...
  const inputMap = {
    'ai-no-5k': 'ai-time5k',
    'ai-no-10k': 'ai-time10k',
    'ai-no-21k': 'ai-time21k',
    'ai-no-42k': 'ai-time42k'
  };

  const input = document.getElementById(inputMap[id]);
  if (input) {
    input.disabled = Boolean(checked);
    input.classList.toggle('disabled', Boolean(checked));
  }
}

function populateAIFormWithProfile(profile = {}) {
  if (!profile || typeof profile !== 'object') return;

  setInputValueIfPresent('ai-age', profile.age);
  setInputValueIfPresent('ai-height', profile.height);
  setInputValueIfPresent('ai-weight', profile.weight);
  setInputValueIfPresent('ai-custom-distance', profile.customDistance);
  setInputValueIfPresent('ai-start-date', profile.startDate);
  setInputValueIfPresent('ai-race-date', profile.raceDate);
  setInputValueIfPresent('ai-time5k', profile.time5k);
  setInputValueIfPresent('ai-time10k', profile.time10k);
  setInputValueIfPresent('ai-time21k', profile.time21k);
  setInputValueIfPresent('ai-time42k', profile.time42k);
  setInputValueIfPresent('ai-test3km-time', profile.test3kmTime);
  setInputValueIfPresent('ai-objective', profile.objective);
  ...
  if (profile.targetDistance) {
    ...
    if (distance) distance.value = profile.targetDistance;
  }

  if (profile.terrain) {
    const terrain = document.querySelector(`input[name="ai-terrain"][value="${profile.terrain}"]`);
    if (terrain) terrain.checked = true;
  }

  if (profile.daysPerWeek) {
    const days = document.getElementById('ai-days');
    if (days) days.value = profile.daysPerWeek;
  }

  setCheckboxIfPresent('ai-no-5k', profile.no5k);
  setCheckboxIfPresent('ai-no-10k', profile.no10k);
  setCheckboxIfPresent('ai-no-21k', profile.no21k);
  setCheckboxIfPresent('ai-no-42k', profile.no42k);

  if (typeof toggleCustomDistance === 'function') toggleCustomDistance();
  if (typeof updateWeeksInfo === 'function') updateWeeksInfo();
}
...
function renderAICoachPage() {
  ...
  const draftProfile = AICoach.loadProfile();

  if (draftProfile && typeof populateAIFormWithProfile === 'function') {
    populateAIFormWithProfile(draftProfile);
  }

  if (localProfile) {
    // Dados do perfil atual completam campos ainda vazios, sem apagar o rascunho da planilha.
    const ageEl = document.getElementById('ai-age');
    const heightEl = document.getElementById('ai-height');
    const weightEl = document.getElementById('ai-weight');

    if (localProfile.age && ageEl && !ageEl.value) ageEl.value = localProfile.age;
    if (localProfile.height && heightEl && !heightEl.value) heightEl.value = localProfile.height;
    if (localProfile.weight && weightEl && !weightEl.value) weightEl.value = localProfile.weight;
  }
...
function resetAIFormAndGenerateAnother() {
  // Não limpar o formulário: o atleta quer gerar outra opção sem redigitar tudo.
  const draft = AICoach.loadProfile();
  pageHistory.length = 0;
  showPage('ai');

  setTimeout(() => {
    if (draft && typeof populateAIFormWithProfile === 'function') populateAIFormWithProfile(draft);
...
function showRunEvoGenerateWithActivePlanModal() {
  return new Promise(resolve => {
    ...
    if (!modal || !iconEl || !titleEl || !messageEl || !confirmBtn) {
      const replace = window.confirm('Você já possui uma planilha ativa. No Free, gerar uma nova planilha substitui a atual. Deseja substituir?');
      resolve(replace ? 'replace' : 'cancel');
      return;
    }

    iconEl.textContent = '⚡';
    titleEl.textContent = 'Você já tem uma planilha ativa';
    messageEl.innerHTML = `
      <div class="generation-paywall-choice">
        <p>No plano Free, gerar uma nova planilha substitui a planilha atual.</p>
        <div class="generation-choice-card free">
          <strong>Substituir no Free</strong>
          <span>A planilha atual será removida e a nova ocupará o lugar dela.</span>
          <button type="button" data-generation-choice="replace">Substituir planilha atual</button>
        </div>
        <div class="generation-choice-card plus">
          <strong>Comparar com RunEvo+</strong>
          <span>Guarde a atual, gere uma nova versão e compare antes de adotar.</span>
          <button type="button" data-generation-choice="plus">Assinar RunEvo+</button>
        </div>
      </div>
    `;
    ...
    messageEl.querySelector('[data-generation-choice="replace"]')?.addEventListener('click', () => {
      modal.classList.add('hidden');
      confirmBtn.classList.remove('hidden');
      resolve('replace');
    });

    messageEl.querySelector('[data-generation-choice="plus"]')?.addEventListener('click', () => {
      modal.classList.add('hidden');
      confirmBtn.classList.remove('hidden');
      resolve('plus');
    });
...
function restoreRunEvoModalDefaultButtons() {
  const confirmBtn = document.getElementById('modal-confirm');
  const cancelBtn = document.getElementById('modal-cancel');
...
async function handleGeneratePlan() {
  ...
  const hasActivePlan = AICoach.isPlanAdopted();
  let shouldArchiveCurrentPlan = false;

  if (hasActivePlan) {
    if (!isRunEvoPlusUser()) {
      const decision = await showRunEvoGenerateWithActivePlanModal();
      restoreRunEvoModalDefaultButtons();

      if (decision === 'plus') {
        showRunEvoPlusModal('planComparison');
        return;
      }
      ...
      if (typeof showToast === 'function') {
        showToast('A nova planilha substituirá a planilha atual no plano Free.', 'info');
      }
    } else {
      shouldArchiveCurrentPlan = true;
      if (typeof showToast === 'function') {
        showToast('RunEvo+: a planilha atual será preservada para comparação.', 'info');
      }
    }
  }
  ...
  try {
    ...
    setAILoadingStep(2);
    if (shouldArchiveCurrentPlan) {
      archiveCurrentPlanForPlus('substituída por nova planilha gerada');
    }
...
function getValidationCodeLabel(code) {
  const labels = {
    ...
    QUALITY_SCORE_LOW: 'Score técnico baixo',
    POST_RECOVERY_VOLUME_CAPPED: 'Progressão pós-recuperação ajustada'
  };
...
function getPlanReviewSummary(plan) {
  ...
  const distanceKm = Number(validationSummary.raceDistanceKm || plan?.targetDistanceKm || plan?.distanceKm || plan?.userData?.customDistance || 0);
  ...
  return {
    ...
    biggestTrainingLongRunKm: validationSummary.biggestTrainingLongRunKm ?? validationSummary.peakTrainingLongRunKm ?? validationSummary.peakLongRunKm ?? validationSummary.biggestLongRunKm ?? Math.max(...trainingLongRuns, 0),
    biggestLongRunKm: validationSummary.biggestTrainingLongRunKm ?? validationSummary.peakTrainingLongRunKm ?? validationSummary.peakLongRunKm ?? validationSummary.biggestLongRunKm ?? Math.max(...trainingLongRuns, 0),
...
function compactText(value = '', maxChars = 145) {
...
function normalizeShortLevel(level = '') {
  const text = String(level || '').toLowerCase();
  if (text.includes('avanç')) return 'Avançado';
  if (text.includes('inic')) return 'Iniciante';
  if (text.includes('inter')) return 'Intermediário';
  return compactText(level, 80) || 'Intermediário';
}
...
function renderCoachAnalysis(plan) {
  ...
  const finalRisk = plan?.validation?.summary?.riskLevel || analysis.riskLevel || blueprint.profile?.riskLevel;
  const riskClass = getRiskLabelClass(finalRisk);
  const riskReasons = plan?.validation?.summary?.riskReasons || analysis.riskReasons || [];
  ...
  return `
    <div class="coach-analysis-card">
      <div class="coach-analysis-header">
        ...
        <span class="coach-risk-pill ${riskClass}">Risco: ${escapeHTML(getRiskLabelText(finalRisk))}</span>
      </div>

      <p class="coach-summary">${escapeHTML(compactText(analysis.coachSummary || 'Plano ajustado ao objetivo, nível e prazo informado.', 170))}</p>
...
function getQualityStatusLabel(status = '') {
  ...
  if (value === 'atenção') return 'Atenção';
  return 'Revisar';
}

function getQualityStatusClass(status = '') {
  const value = String(status || '').toLowerCase();
  if (value === 'excelente') return 'excellent';
  if (value === 'boa') return 'good';
  if (value === 'atenção') return 'warning';
  return 'danger';
}

function renderQualityScore(plan) {
  const quality = plan?.validation?.quality;
  if (!quality) return '';

  const metrics = quality.metrics || {};
  const metricItems = [
    ['Variedade', metrics.variety],
    ['Progressão', metrics.progression],
    ['Longões', metrics.longRunBalance],
    ['Intensidade', metrics.intensityDistribution],
    ['Recuperação', metrics.recovery],
    ['Fases', metrics.phaseIdentity]
  ];

  return `
    <div class="plan-quality-card ${getQualityStatusClass(quality.status)}">
      <div class="plan-quality-head">
        <div>
          <span class="plan-quality-eyebrow">Auditoria técnica</span>
          <h4>Quality Score</h4>
          <p>Leitura automática de coerência, variedade e segurança da planilha.</p>
        </div>
        <div class="plan-quality-score">
          <strong>${Number(quality.overall || 0).toFixed(1)}</strong>
          <span>/10</span>
          <small>${escapeHTML(getQualityStatusLabel(quality.status))}</small>
        </div>
      </div>

      <div class="plan-quality-metrics">
        ${metricItems.map(([label, value]) => `
          <div class="plan-quality-metric">
            <span>${escapeHTML(label)}</span>
            <strong>${Number(value || 0).toFixed(1)}</strong>
            <div class="plan-quality-bar"><i style="width:${Math.max(0, Math.min(100, Number(value || 0) * 10))}%"></i></div>
          </div>
        `).join('')}
      </div>

      <div class="plan-quality-insights">
        ${quality.adoptionAdvice ? `<p class="plan-quality-advice">📌 ${escapeHTML(quality.adoptionAdvice)}</p>` : ''}
        ${(quality.insights || []).slice(0, 5).map(item => `<p>• ${escapeHTML(item)}</p>`).join('')}
      </div>
      ${quality.details?.raceWeekIgnored ? `<div class="plan-quality-note">Semana da prova ignorada na penalização de longão comum.</div>` : ''}
    </div>
  `;
}
  ...
  return `
    <div class="plan-review-card">
      ...
      <div class="plan-review-grid">
        ...
        <div class="plan-review-metric">
          <span>Maior longão de treino</span>
          <strong>${formatKm(summary.biggestTrainingLongRunKm || summary.biggestLongRunKm)}</strong>
        </div>
      </div>
...
function renderAIPlanResult(plan) {
  ...
  const qualityStr = plan.validation?.quality?.overall ? ` • Score: ${Number(plan.validation.quality.overall).toFixed(1)}/10` : '';
  const validationStr = plan.validation
    ? ` • Validação: ${plan.validation.status === 'ok' ? 'OK' : `${plan.validation.summary.totalFixes} ajuste(s)`}${qualityStr}`
    : '';
  ...
  const plusPitch = !isRunEvoPlusUser() ? `
    <div class="plus-inline-card">
      <div>
        <span class="plus-inline-eyebrow">RunEvo+</span>
        <strong>Compare versões antes de trocar de planilha</strong>
        <p>Histórico e comparação de planilhas serão recursos premium. No Free, mantenha uma planilha ativa por vez.</p>
      </div>
      <button type="button" onclick="showRunEvoPlusModal('planComparison')">Ver benefício</button>
    </div>
  ` : `
    <div class="plus-inline-card unlocked">
      <div>
        <span class="plus-inline-eyebrow">RunEvo+</span>
        <strong>Histórico e comparação liberados</strong>
        <p>Estrutura pronta para comparar versões e arquivar planilhas antigas.</p>
      </div>
      <button type="button" onclick="showRunEvoPlusModal('planHistory')">Gerenciar em breve</button>
    </div>
  `;

  const comparisonHtml = renderPlanComparisonHTML(plan);
...
function handleAdoptPlan() {
  ...
  confirmBtn.onclick = () => {
    try {
      if (isRunEvoPlusUser() && AICoach.isPlanAdopted()) {
        archiveCurrentPlanForPlus('substituída por nova planilha adotada');
      }
      ...
      window.RunEvoCloudDataService?.syncActivePlan?.(AICoach.loadPlan())
        .then(result => {
          if (result?.synced) showToast('Planilha adotada e sincronizada na nuvem.', 'success');
          else showToast('Planilha adotada com sucesso.', 'success');
        })
        .catch(error => {
          console.warn('RunEvo: planilha não sincronizada na nuvem.', error);
          showToast('Planilha adotada localmente. Sincronização pendente.', 'info');
        });
    } catch (error) {
...
function handleUnadoptPlan() {
  ...
  confirmBtn.onclick = () => {
    try {
      if (isRunEvoPlusUser()) {
        archiveCurrentPlanForPlus('removida manualmente');
      }
      AICoach.unadoptPlan();
      clearProgress();
      restoreOriginalPlan();
      ...
      if (typeof showToast === 'function') showToast('Planilha removida. Agora você pode gerar uma nova.', 'info');
    } catch (error) {
  ...
  if (AICoach.isPlanAdopted()) {
    ...
    if (plan) {
      ...
      document.getElementById('adopted-plan-detail').textContent =
        `${plan.totalWeeks} semanas • ${plan.raceName} • ${isRunEvoPlusUser() ? 'histórico RunEvo+ disponível' : 'Free: remova para gerar outra'}`;
    }

    banner.classList.remove('hidden');
    if (isRunEvoPlusUser()) {
      generateBtn?.classList.remove('disabled-by-active-plan');
      if (generateBtn) generateBtn.title = 'RunEvo+: gere uma nova planilha para comparar com a atual.';
    } else {
  ...
  document.getElementById('modal-message').innerHTML = `
    <form class="manual-plan-form" onsubmit="return false;">
      ...
      <div class="manual-editor-warning">
        Alterações manuais são salvas na planilha atual, aparecem no app, no PDF e no XLS.
      </div>
    </form>
  `;
  ...
  document.getElementById('modal-message').innerHTML = `
    <form class="manual-plan-form" onsubmit="return false;">
      <div class="manual-editor-context">
        ...
        <span>O novo treino será salvo no planilha atual e entrará nos relatórios.</span>
      </div>
...
function confirmRemoveWorkout(id) {
  ...
  document.getElementById('modal-message').innerHTML = `
    <div class="manual-remove-warning">
      ...
      <p>Essa ação remove o treino do planilha atual, dos relatórios PDF/XLS. Progresso registrado neste treino também será apagado.</p>
    </div>
  `;
...
document.getElementById('conta-import-input')?.addEventListener('change', (event) => {
...
function getPlanDistanceLabel(plan) {
  const km = Number(plan?.validation?.summary?.raceDistanceKm || plan?.targetDistanceKm || plan?.distanceKm || plan?.distance || 0);
  return km ? `${Math.round(km * 10) / 10} km` : (plan?.raceName || '-');
}

function getCompactPlanSummary(plan) {
  const summary = getPlanReviewSummary(plan || {});
  return {
    name: plan?.planName || plan?.raceName || 'Planilha RunEvo',
    race: plan?.raceName || getPlanDistanceLabel(plan),
    weeks: plan?.totalWeeks || summary.totalWeeks || '-',
...
function renderPlanMiniCard(plan, label = 'Planilha', mode = 'default') {
  const s = getCompactPlanSummary(plan);
  const isHistory = mode === 'history';
  const compactLabel = String(label || 'Planilha')
    .replace('substituída por nova planilha adotada', 'arquivada')
    .replace('Substituída por nova planilha adotada', 'arquivada')
    .replace('removida manualmente', 'removida')
    .replace('Removida manualmente', 'removida')
    .replace('Planilha ativa', 'ativa')
    .replace('planilha ativa', 'ativa');

  const metrics = [
    `Score ${escapeHTML(s.score)}`,
    `Risco ${escapeHTML(s.risk)}`,
    `Pico ${escapeHTML(s.peak)}`,
    `Longão ${escapeHTML(s.long)}`
  ];

  const metricsHtml = isHistory
    ? `<div class="history-plan-metrics">${metrics.map(item => `<b>${item}</b>`).join('')}</div>`
    : `<div class="plus-plan-mini-grid">${metrics.map(item => `<b>${item}</b>`).join('')}</div>`;

  return `
    <article class="plus-plan-mini-card ${isHistory ? 'history-card history-card-v140' : ''}">
      <span title="${escapeHTML(label)}">${escapeHTML(compactLabel)}</span>
      <strong title="${escapeHTML(s.name)}">${escapeHTML(s.name)}</strong>
      <p>${escapeHTML(s.race)} • ${escapeHTML(String(s.weeks))} semanas</p>
      ${metricsHtml}
    </article>
  `;
}
...
function parseRunEvoNumber(value, fallback = 0) {
  if (typeof value === 'number') return Number.isFinite(value) ? value : fallback;
  const parsed = Number(String(value ?? '').replace(',', '.').replace(/[^\d.-]/g, ''));
  return Number.isFinite(parsed) ? parsed : fallback;
}

function getRunEvoRiskWeight(risk = '') {
  const value = String(risk || '').toLowerCase();
  if (value.includes('muito alto')) return 4;
  if (value.includes('alto')) return 3;
  if (value.includes('médio') || value.includes('medio') || value.includes('moderado')) return 2;
  if (value.includes('baixo')) return 1;
  return 2;
}

function getRunEvoPlanComparisonMetrics(plan) {
  const summary = getPlanReviewSummary(plan || {});
  const quality = plan?.validation?.quality || {};
  ...
  return {
    compact,
    score: parseRunEvoNumber(quality.overall ?? compact.score, 0),
    riskWeight: getRunEvoRiskWeight(compact.risk),
    risk: compact.risk,
    peakKm: parseRunEvoNumber(summary.peakWeeklyKm || plan?.peakWeekKm || compact.peak, 0),
    longKm: parseRunEvoNumber(summary.biggestTrainingLongRunKm || plan?.longestTrainingRun || compact.long, 0),
    variety: parseRunEvoNumber(quality.variety, 0),
    progression: parseRunEvoNumber(quality.progression, 0),
    longRuns: parseRunEvoNumber(quality.longRuns, 0),
    intensity: parseRunEvoNumber(quality.intensity, 0),
    recovery: parseRunEvoNumber(quality.recovery, 0),
    phases: parseRunEvoNumber(quality.phases, 0),
    totalWeeks: parseRunEvoNumber(plan?.totalWeeks || summary.totalWeeks, 0),
    totalKm: parseRunEvoNumber(summary.totalKm || plan?.totalKm || plan?.totalDistanceKm, 0)
  };
}

function getRunEvoMetricWinner(currentValue, newValue, mode = 'higher') {
  const a = Number(currentValue || 0);
  const b = Number(newValue || 0);
  const diff = Math.abs(a - b);

  if (diff < 0.05) return 'empate';

  if (mode === 'lower') return b < a ? 'nova' : 'atual';
  if (mode === 'balanced') {
    return Math.abs(b) <= Math.abs(a) ? 'nova' : 'atual';
  }

  return b > a ? 'nova' : 'atual';
}

function getRunEvoWinnerLabel(winner) {
  if (winner === 'nova') return 'Nova';
  if (winner === 'atual') return 'Atual';
  return 'Equilíbrio';
}

function renderRunEvoComparisonMetric(label, currentValue, newValue, winner, note = '') {
  return `
    <article class="pro-compare-metric winner-${escapeHTML(winner)}">
      <span>${escapeHTML(label)}</span>
      <div class="pro-compare-values">
        <b>Atual <strong>${escapeHTML(String(currentValue))}</strong></b>
        <b>Nova <strong>${escapeHTML(String(newValue))}</strong></b>
      </div>
      <p>Leitura: ${escapeHTML(getRunEvoWinnerLabel(winner))}${note ? ` • ${escapeHTML(note)}` : ''}</p>
    </article>
  `;
}

function getRunEvoComparisonRecommendation(currentMetrics, newMetrics) {
  let points = 0;
  const reasons = [];

  if (newMetrics.score >= currentMetrics.score + 0.3) {
    points += 2;
    reasons.push('score técnico melhor');
  } else if (newMetrics.score < currentMetrics.score - 0.5) {
    points -= 2;
    reasons.push('score técnico inferior');
  }

  if (newMetrics.riskWeight < currentMetrics.riskWeight) {
    points += 2;
    reasons.push('risco menor');
  } else if (newMetrics.riskWeight > currentMetrics.riskWeight) {
    points -= 2;
    reasons.push('risco maior');
  }

  if (newMetrics.progression >= currentMetrics.progression + 0.5) {
    points += 1;
    reasons.push('progressão mais segura');
  } else if (newMetrics.progression < currentMetrics.progression - 1) {
    points -= 1;
    reasons.push('progressão pior');
  }

  if (newMetrics.variety >= currentMetrics.variety + 0.5) {
    points += 1;
    reasons.push('maior variedade');
  }

  if (newMetrics.longRuns >= currentMetrics.longRuns + 0.5) {
    points += 1;
    reasons.push('longões melhor calibrados');
  } else if (newMetrics.longRuns < currentMetrics.longRuns - 1) {
    points -= 1;
    reasons.push('longões exigem revisão');
  }

  if (points >= 3) {
    return {
      status: 'adotar',
      title: 'Recomendação RunEvo: adotar a nova planilha',
      text: `A nova versão parece tecnicamente superior: ${reasons.slice(0, 3).join(', ') || 'melhor equilíbrio geral'}.`
    };
  }
  ...
  if (points <= -2) {
    return {
      status: 'manter',
      title: 'Recomendação RunEvo: manter a atual por enquanto',
      text: `A nova versão não superou a atual com segurança: ${reasons.slice(0, 3).join(', ') || 'há pontos técnicos a revisar'}.`
    };
  }

  return {
    status: 'revisar',
    title: 'Recomendação RunEvo: revisar antes de adotar',
    text: `As duas versões estão próximas. Compare objetivo, risco e sensação de carga antes de trocar.`
  };
}



function normalizeRunEvoComparablePlan(plan = {}) {
  const weeks = Array.isArray(plan.weeks) ? plan.weeks : [];
  const workouts = weeks.flatMap(week => {
    ...
    return ws.map(w => ({
      ...
      description: String(w.description || w.desc || '').replace(/\s+/g, ' ').trim().toLowerCase()
    }));
  });
  ...
  return {
    objective: String(plan.objective || plan.goal || '').replace(/\s+/g, ' ').trim().toLowerCase(),
    race: String(compact.race || '').toLowerCase(),
    weeks: Number(plan.totalWeeks || compact.weeks || weeks.length || 0),
    workoutsCount: workouts.length,
    totalKm: Number(summary.totalKm || plan.totalKm || plan.totalDistanceKm || 0),
...
function getRunEvoPlanFingerprint(plan = {}) {
  const comparable = normalizeRunEvoComparablePlan(plan);
  return JSON.stringify(comparable);
}

function areRunEvoPlansIdentical(currentPlan, newPlan) {
  if (!currentPlan || !newPlan) return false;
  return getRunEvoPlanFingerprint(currentPlan) === getRunEvoPlanFingerprint(newPlan);
}

function renderRunEvoIdenticalPlanComparison(currentSummary, newSummary) {
  return `
    <section class="plus-plan-compare-card pro-comparison-card identical-plan-card">
      <span class="plus-inline-eyebrow">RunEvo+ comparação técnica</span>
      <h3>Planilha nova idêntica à atual</h3>
      <p class="pro-comparison-lead">O Motor Evo identificou que a nova geração não trouxe diferença relevante em relação à planilha ativa.</p>

      <div class="pro-comparison-hero">
        <article>
          <span>Atual</span>
          <strong>${escapeHTML(currentSummary.name)}</strong>
          <p>${escapeHTML(currentSummary.race)} • ${escapeHTML(String(currentSummary.weeks))} semanas</p>
        </article>
        <article class="new">
          <span>Nova</span>
          <strong>${escapeHTML(newSummary.name)}</strong>
          <p>${escapeHTML(newSummary.race)} • ${escapeHTML(String(newSummary.weeks))} semanas</p>
        </article>
      </div>

      <div class="identical-plan-notice">
        <span>🧬</span>
        <div>
          <strong>A planilha nova é idêntica à atual.</strong>
          <p>Não faz sentido substituir ou adotar uma cópia. Altere objetivo, prazo, frequência, terreno ou métricas para gerar uma versão realmente diferente.</p>
        </div>
      </div>

      <div class="pro-comparison-actions single">
        <button type="button" onclick="showPage('ai-coach')">Ajustar dados e gerar outra</button>
      </div>
    </section>
  `;
}

function renderPlanComparisonHTML(newPlan) {
  if (!isRunEvoPlusUser() || !AICoach.isPlanAdopted()) return '';

  const current = AICoach.loadPlan();
  if (!current || !newPlan || current === newPlan) return '';

  const currentSummary = getCompactPlanSummary(current);
  const newSummary = getCompactPlanSummary(newPlan);

  if (areRunEvoPlansIdentical(current, newPlan)) {
    return renderRunEvoIdenticalPlanComparison(currentSummary, newSummary);
  }

  const currentMetrics = getRunEvoPlanComparisonMetrics(current);
  const newMetrics = getRunEvoPlanComparisonMetrics(newPlan);
  const recommendation = getRunEvoComparisonRecommendation(currentMetrics, newMetrics);

  const scoreWinner = getRunEvoMetricWinner(currentMetrics.score, newMetrics.score, 'higher');
  const riskWinner = getRunEvoMetricWinner(currentMetrics.riskWeight, newMetrics.riskWeight, 'lower');
  const varietyWinner = getRunEvoMetricWinner(currentMetrics.variety, newMetrics.variety, 'higher');
  const progressionWinner = getRunEvoMetricWinner(currentMetrics.progression, newMetrics.progression, 'higher');
  const longWinner = getRunEvoMetricWinner(currentMetrics.longRuns, newMetrics.longRuns, 'higher');
  const peakWinner = getRunEvoMetricWinner(
    Math.abs(currentMetrics.peakKm - newMetrics.peakKm),
    0,
    'lower'
  );

  return `
    <section class="plus-plan-compare-card pro-comparison-card">
      <span class="plus-inline-eyebrow">RunEvo+ comparação técnica</span>
      <h3>Atual x nova planilha</h3>
      <p class="pro-comparison-lead">Leitura objetiva para decidir se vale trocar, manter ou revisar antes de adotar.</p>
      ...
      <div class="pro-comparison-grid">
        ${renderRunEvoComparisonMetric('Score técnico', currentSummary.score, newSummary.score, scoreWinner, 'quanto maior, melhor')}
        ${renderRunEvoComparisonMetric('Risco', currentSummary.risk, newSummary.risk, riskWinner, 'quanto menor, melhor')}
        ${renderRunEvoComparisonMetric('Variedade', currentMetrics.variety.toFixed(1).replace('.', ','), newMetrics.variety.toFixed(1).replace('.', ','), varietyWinner, 'evita planilha repetitiva')}
        ${renderRunEvoComparisonMetric('Progressão', currentMetrics.progression.toFixed(1).replace('.', ','), newMetrics.progression.toFixed(1).replace('.', ','), progressionWinner, 'segurança de carga')}
        ${renderRunEvoComparisonMetric('Longões', currentMetrics.longRuns.toFixed(1).replace('.', ','), newMetrics.longRuns.toFixed(1).replace('.', ','), longWinner, 'alinhamento com a prova')}
        ${renderRunEvoComparisonMetric('Volume pico', currentSummary.peak, newSummary.peak, peakWinner, 'analise tolerância à carga')}
      </div>

      <div class="pro-comparison-recommendation ${escapeHTML(recommendation.status)}">
        <span>📌</span>
        <div>
      ...
      <div class="pro-comparison-actions">
        <button type="button" onclick="handleAdoptPlan()">Adotar nova planilha</button>
        <button type="button" class="secondary" onclick="showPage('home')">Manter atual</button>
      </div>

      <p class="pro-comparison-footnote">Ao adotar a nova planilha, a atual será arquivada no histórico RunEvo+.</p>
    </section>
  `;
}

function renderRunEvoPlusHistory() {
  const holder = document.getElementById('runevo-plus-history');
  if (!holder) return;

  const activePlan = typeof AICoach !== 'undefined' ? AICoach.loadPlan?.() : null;
  const rawHistory = loadRunEvoPlanHistory();
  const history = normalizeRunEvoPlanHistory(rawHistory, activePlan);

  if (!isRunEvoPlusUser()) {
    holder.innerHTML = `
      <div class="plus-history-locked">
        <span>🔒</span>
        <strong>Histórico e comparação são RunEvo+</strong>
        <p>No Free, você mantém uma planilha ativa por vez. No RunEvo+, pode guardar versões e comparar antes de trocar.</p>
        <button type="button" onclick="showRunEvoPlusModal('planHistory')">Ver benefício</button>
      </div>
    `;
    return;
  }
  ...
  holder.innerHTML = `
    <div class="plus-history-current">
      ${activePlan ? renderPlanMiniCard(activePlan, 'Planilha ativa', 'history') : '<p class="plus-empty-history">Nenhuma planilha ativa no momento.</p>'}
    </div>
    <div class="plus-history-list" role="list" aria-label="Planilhas arquivadas">
      ${history.length ? history.slice(0, 12).map(item => renderPlanMiniCard(item.plan || item, item.reason || 'Arquivada', 'history')).join('') : '<p class="plus-empty-history">Nenhuma planilha arquivada ainda. Ao trocar/remover uma planilha, ela entra aqui.</p>'}
      ${history.length ? '<div class="plus-history-scroll-end" aria-hidden="true"></div>' : ''}
    </div>
  `;
}


function openMyResourcesPage() {
  renderMyResourcesPage();
  showPage('my-resources');
}


function openRunEvoPlusPage() {
  renderRunEvoPlusPage();
  showPage('runevo-plus');
}

function getRunEvoPlusFeatureRows() {
  return getRunEvoVisibleResources();
}



let selectedRunEvoBillingPlan = 'annual';

function selectRunEvoBillingPlan(plan = 'annual') {
  selectedRunEvoBillingPlan = String(plan || 'annual') === 'monthly' ? 'monthly' : 'annual';

  saveRunEvoSubscriptionState({ billing: selectedRunEvoBillingPlan });
  document.querySelectorAll('[data-billing-plan]').forEach(card => {
    const active = card.dataset.billingPlan === selectedRunEvoBillingPlan;
    card.classList.toggle('selected', active);
    card.setAttribute('aria-pressed', active ? 'true' : 'false');
  });

  const cta = document.getElementById('runevo-plus-cta');
  if (cta && !isRunEvoPlusUser()) {
    cta.textContent = selectedRunEvoBillingPlan === 'annual'
      ? 'Assinar RunEvo+ anual'
      : 'Assinar RunEvo+ mensal';
  }
}


function renderRunEvoPlusPage() {
  const plan = getRunEvoSubscriptionPlan();
  const isPlus = plan === 'plus';
  const details = getRunEvoPlanDetails();
  const logo = document.getElementById('runevo-plus-page-logo');
  const status = document.getElementById('runevo-plus-status');
  const cta = document.getElementById('runevo-plus-cta');
  const comparison = document.getElementById('runevo-plus-sales-comparison');
  const pricing = document.getElementById('runevo-plus-pricing');
  const features = document.getElementById('my-resources');
  const currentPlan = document.getElementById('runevo-plus-current-plan');
  const currentDesc = document.getElementById('runevo-plus-current-description');
  const policyTable = document.getElementById('runevo-plus-policy-table');

  if (logo) {
    logo.src = isPlus ? 'assets/img/logo-runevo-plus.png' : 'assets/img/logo-runevo.png';
    logo.alt = isPlus ? 'RunEvo+' : 'RunEvo';
  }

  if (status) {
    status.textContent = isPlus ? 'Assinatura ativa' : 'Plano Free';
    status.classList.toggle('active', isPlus);
  }

  if (currentPlan) currentPlan.textContent = details.label;
  if (currentDesc) currentDesc.textContent = details.description;

  if (cta) {
    cta.textContent = isPlus ? 'RunEvo+ ativo' : 'Assinar RunEvo+';
    cta.disabled = isPlus;
    cta.onclick = () => {
      if (isPlus) return;
      const planText = selectedRunEvoBillingPlan === 'annual' ? 'anual de R$ 119,90/ano' : 'mensal de R$ 15,00/mês';
      showSimpleModal(
        '⚡',
        'RunEvo+ em preparação',
        `Você selecionou o plano ${planText}. A próxima etapa será conectar a assinatura real via loja/app ou provedor de pagamento.`
      );
    };
  }

  if (features) {
    features.innerHTML = getRunEvoPlusFeatureRows().map(([icon, title, text]) => `
      <div class="plus-feature-row">
        <span class="plus-feature-icon">${escapeHTML(icon)}</span>
        <div>
          <strong>${escapeHTML(title)}</strong>
          <p>${escapeHTML(text)}</p>
        </div>
  ...
  if (comparison) {
    comparison.innerHTML = RunEvo_SALES_COMPARISON.map(([feature, free, plus]) => `
      <div class="plus-policy-card">
        <strong>${escapeHTML(feature)}</strong>
        <div class="plus-policy-options">
          <span class="free-option"><small>Free</small>${escapeHTML(free)}</span>
          <span class="plus-option"><small>RunEvo+</small>${escapeHTML(plus)}</span>
        </div>
      </div>
    `).join('');
  }
  ...
  if (pricing) {
    pricing.innerHTML = `
      <button type="button" class="plus-price-card featured" data-billing-plan="annual" onclick="selectRunEvoBillingPlan('annual')" aria-pressed="false">
        <span>Anual</span>
        <em>Economize 33%</em>
        <strong>R$ 119,90/ano</strong>
        <p>Equivale a R$ 9,99/mês. Melhor escolha para quem treina o ano todo.</p>
        <small>Melhor custo-benefício</small>
      </button>
      <button type="button" class="plus-price-card" data-billing-plan="monthly" onclick="selectRunEvoBillingPlan('monthly')" aria-pressed="false">
        <span>Mensal</span>
        <strong>R$ 15,00/mês</strong>
        <p>Flexível para testar o RunEvo+ antes de assumir um ciclo maior.</p>
        <small>Cancele quando quiser</small>
      </button>
    `;
    requestAnimationFrame(() => selectRunEvoBillingPlan(selectedRunEvoBillingPlan || 'annual'));
  }
...
function renderMyResourcesPage() {
  const plan = getRunEvoSubscriptionPlan();
  const isPlus = plan === 'plus';
  const features = document.getElementById('my-resources-list');
  const policyTable = document.getElementById('my-resources-policy-table');
  const history = document.getElementById('my-resources-history');
  const historySection = document.getElementById('my-resources-history-section');
  const noteSection = document.getElementById('my-resources-note-section');
  const upgradeSection = document.getElementById('my-resources-upgrade-section');
  const lockedResources = document.getElementById('my-resources-locked-list');
  const resourcesSubtitle = document.getElementById('my-resources-subtitle');
  const badge = document.getElementById('my-resources-status');
  const planTitle = document.getElementById('my-resources-plan-title');
  const planDescription = document.getElementById('my-resources-plan-description');
  const planChip = document.getElementById('my-resources-plan-chip');

  if (badge) {
    badge.textContent = isPlus ? `RunEvo+ ${getRunEvoBillingLabel()}` : 'Plano Free';
    badge.classList.toggle('active', isPlus);
  }

  if (resourcesSubtitle) {
    resourcesSubtitle.textContent = getRunEvoResourceFooterCopy();
  }

  if (planTitle) planTitle.textContent = isPlus ? 'Plano RunEvo+' : 'Plano Free';
  if (planDescription) {
    planDescription.textContent = isPlus
      ? 'Histórico, comparação e recursos Pro liberados para controlar melhor sua evolução.'
      : 'Sua jornada essencial para treinar, registrar e evoluir com uma planilha ativa.';
  }
  if (planChip) planChip.textContent = isPlus ? 'Recursos Pro ativos' : '1 planilha ativa';

  if (features) {
    const visibleResources = getRunEvoVisibleResources();
    features.innerHTML = visibleResources.map(([icon, title, text]) => `
      <div class="plus-feature-row">
        <span class="plus-feature-icon">${escapeHTML(icon)}</span>
        <div>
          <strong>${escapeHTML(title)}</strong>
  ...
  if (historySection) historySection.classList.toggle('hidden', !isPlus);
  if (upgradeSection) upgradeSection.classList.toggle('hidden', isPlus);
  if (lockedResources) lockedResources.innerHTML = isPlus ? '' : renderRunEvoLockedResources();
  if (noteSection) noteSection.classList.toggle('hidden', false);

  updateRunEvoPremiumUI?.();

  if (history) {
    if (isPlus) {
      renderRunEvoPlusHistoryInto(history);
    } else {
      history.innerHTML = '';
    }
  }
...
function getPlanHistorySignature(plan) {
  const s = getCompactPlanSummary(plan || {});
  return [
    String(s.name || '').toLowerCase().trim(),
    String(s.race || '').toLowerCase().trim(),
    String(s.weeks || '').trim(),
    String(s.score || '').trim(),
    String(s.peak || '').trim(),
    String(s.long || '').trim()
...
function normalizeRunEvoPlanHistory(history = [], activePlan = null) {
  const activeSig = activePlan ? getPlanHistorySignature(activePlan) : '';
  const seen = new Set();
  const normalized = [];
  ...
  (Array.isArray(history) ? history : []).forEach(item => {
    ...
    const sig = getPlanHistorySignature(plan);
    if (!sig || sig === activeSig || seen.has(sig)) return;
...
function auditRunEvoHistoryRenderer() {
  const cards = Array.from(document.querySelectorAll('.runevo-plus-history .plus-plan-mini-card'));
  const report = {
    build: typeof RunEvo_BUILD_VERSION !== 'undefined' ? RunEvo_BUILD_VERSION : 'unknown',
    cards: cards.length,
    historyCards: document.querySelectorAll('.runevo-plus-history .history-card-v140').length,
    oldGridsInsideHistory: document.querySelectorAll('.runevo-plus-history .plus-plan-mini-grid').length,
    newMetricsInsideHistory: document.querySelectorAll('.runevo-plus-history .history-plan-metrics').length
  };
  console.table(report);
  return report;
}

function renderRunEvoPlusHistoryInto(holder) {
  if (!holder) return;

  const activePlan = typeof AICoach !== 'undefined' ? AICoach.loadPlan?.() : null;
  const rawHistory = loadRunEvoPlanHistory();
  const history = typeof normalizeRunEvoPlanHistory === 'function'
    ? normalizeRunEvoPlanHistory(rawHistory, activePlan)
  ...
  setTimeout(() => auditRunEvoHistoryRenderer?.(), 0);
}
...
function renderSettingsPage() {
  ...
  setText('profile-hub-name', getFriendlyProfileName(profile?.displayName, profile?.username) || getFriendlyProfileName(plan?.userData?.name, profile?.username) || 'Atleta');
  setText('profile-joined-date', formatProfileJoinDate(profile));
  setText('profile-app-version', 'versão RunEvo v160');
  setText('settings-adopted', `${summary.isAdopted ? 'Adotada' : 'Livre'} • ${getRunEvoPlanLabel()}`);
  ...
  setValue('settings-athlete-name', getFriendlyProfileName(profile?.displayName, profile?.username) || getFriendlyProfileName(plan?.userData?.name, profile?.username) || '');
  setText('profile-plus-status', getRunEvoPlanLabel());
...
async function saveAthleteProfileSettings() {
  const current = StorageService.loadUserProfile?.() || {};
  const { plan, userData } = getCurrentAthleteMetrics();
  const displayName = document.getElementById('settings-athlete-name')?.value.trim() || '';

  if (!displayName) {
    showSimpleModal('⚠️', 'Nome obrigatório', 'Informe o nome do atleta para salvar o perfil.');
    return;
  }
  ...
  const updatedProfile = {
    ...current,
    displayName,
    avatar: getProfileInitialFromName(displayName),
    age,
    height,
    weight,
    imc,
  ...
  try {
    await window.RunEvoCloudAuthService?.updateUserMetadata?.({
      displayName,
      avatarUrl: updatedProfile.photo || updatedProfile.googleAvatarUrl || ''
    });
    await window.RunEvoCloudDataService?.syncProfile?.(StorageService.loadUserProfile?.() || updatedProfile);
  } catch (error) {
    console.warn('RunEvo: perfil salvo localmente, mas sync cloud falhou.', error);
  }
  ...
  document.getElementById('modal-message').textContent = 'Nome e dados do atleta atualizados.';
  document.getElementById('modal-overlay').classList.remove('hidden');
  document.getElementById('modal-cancel').classList.add('hidden');
...
let authLoginStep = 'email';



const RUNEVO_PENDING_SIGNUPS_KEY = 'runevo_pending_signups_v100';

function savePendingSignupName(email = '', name = '') {
  if (!email || !name) return;
  try {
    const map = JSON.parse(localStorage.getItem(RUNEVO_PENDING_SIGNUPS_KEY) || '{}') || {};
    map[UserProfileService.normalizeUsername(email)] = {
      name: String(name || '').trim(),
      savedAt: new Date().toISOString()
    };
    localStorage.setItem(RUNEVO_PENDING_SIGNUPS_KEY, JSON.stringify(map));
  } catch (error) {
    console.warn('RunEvo: pending signup name não salvo.', error);
  }
}

function consumePendingSignupName(email = '') {
  if (!email) return '';
  try {
    const key = UserProfileService.normalizeUsername(email);
    const map = JSON.parse(localStorage.getItem(RUNEVO_PENDING_SIGNUPS_KEY) || '{}') || {};
...
function getFriendlyProfileName(candidate = '', email = '') {
  ...
  if (lower === 'atleta') return '';
  if (normalizedEmail && lower === normalizedEmail) return '';
  if (normalizedEmail && lower === normalizedEmail.split('@')[0]) return '';

  return raw;
}

function getProfileInitialFromName(name = 'A') {
  return String(name || 'A').trim().charAt(0).toUpperCase() || 'A';
}



function isGenericProfileName(value = '', email = '') {
  const raw = String(value || '').trim().toLowerCase();
  const normalizedEmail = String(email || '').trim().toLowerCase();
  if (!raw) return true;
  if (raw === 'atleta') return true;
  if (normalizedEmail && raw === normalizedEmail) return true;
  if (normalizedEmail && raw === normalizedEmail.split('@')[0]) return true;
  return false;
}
...
function updateAuthStepUI() {
  ...
  const backBtn = document.querySelector('.auth-step-back');
  const submitBtn = document.getElementById('auth-submit');
  const helper = document.getElementById('auth-helper');
  const socials = document.querySelector('.auth-social-grid');
  const divider = document.querySelector('.auth-divider');
  const googleBtn = document.querySelector('.auth-social-btn[onclick*="google"]');
  const appleBtn = document.querySelector('.auth-social-btn[onclick*="apple"]');
  const forgotBtn = document.querySelector('.auth-link-btn');
  ...
  const isForgot = authLoginStep === 'forgot';
  const isReset = authLoginStep === 'reset';
  const isPasswordStep = !isRegister && authLoginStep === 'password';
  const isEmailStep = !isRegister && authLoginStep === 'email';

  if (title) {
    title.textContent = isRegister
      ? 'Inscreva-se no RunEvo'
      : isForgot
        ? 'Alterar sua senha'
        : isReset
          ? 'Defina sua nova senha'
          : (isEmailStep ? 'Qual é o seu endereço de e-mail?' : 'Qual é a sua senha?');
  }

  nameGroup?.classList.toggle('hidden', !isRegister);
  emailGroup?.classList.toggle('hidden', !(isRegister || isEmailStep || isForgot));
  passwordGroup?.classList.toggle('hidden', !(isRegister || isPasswordStep || isReset));
  confirmPasswordGroup?.classList.toggle('hidden', !isReset);
  backBtn?.classList.toggle('hidden', isRegister || isEmailStep || isReset);

  if (emailInput) emailInput.disabled = !(isRegister || isEmailStep || isForgot);
  if (passwordInput) {
    passwordInput.disabled = !(isRegister || isPasswordStep || isReset);
    passwordInput.required = isRegister || isPasswordStep || isReset;
    passwordInput.autocomplete = isRegister || isReset ? 'new-password' : 'current-password';
    passwordInput.placeholder = isReset ? 'Nova senha' : (isRegister ? 'Crie uma senha' : 'Senha');
  }
  ...
  if (submitBtn) {
    submitBtn.textContent = isRegister
      ...
      : isForgot
        ...
        : isReset
          ? 'Redefinir senha'
          : (isEmailStep ? 'Continuar' : 'Fazer login');
  }

  if (helper) {
    helper.textContent = isRegister
      ? 'Ao criar sua conta, você concorda com os Termos e a Política de Privacidade do RunEvo.'
      : isForgot
        ? 'Informe o e-mail cadastrado para receber o link de redefinição.'
        : isReset
          ? 'Crie uma nova senha para acessar sua conta RunEvo.'
          : (isEmailStep ? 'Informe seu e-mail para continuar.' : '');
  }

  if (googleBtn) {
    googleBtn.innerHTML = `<img class="auth-provider-icon" src="assets/img/google-logo.png" alt="" onerror="this.style.display='none'"> ${isRegister ? 'Inscrever-se com o Google' : 'Fazer login com Google'}`;
  }

  if (appleBtn) {
    appleBtn.innerHTML = `<img class="auth-provider-icon" src="assets/img/apple-logo.png" alt="" onerror="this.style.display='none'"> ${isRegister ? 'Inscrever-se com o Apple' : 'Fazer login com Apple'}`;
  }

  const hideSocials = isPasswordStep || isForgot || isReset;
  socials?.classList.toggle('hidden', hideSocials);
  socials?.classList.toggle('auth-social-register', isRegister);
  divider?.classList.toggle('hidden', hideSocials || (!isRegister && !isEmailStep));
  forgotBtn?.classList.toggle('hidden', isRegister || isEmailStep || isForgot || isReset);
}

function resetAuthEmailStep() {
  authLoginStep = 'email';
  updateAuthStepUI();
...
function continueAuthEmailStep() {
  const email = UserProfileService.normalizeUsername(document.getElementById('login-username')?.value || '');
  if (!UserProfileService.isValidEmail(email)) {
    setAuthError('Informe um e-mail válido para continuar.');
    return false;
  }
...
function applyAuthenticatedUser(authUser, fallbackEmail = '') {
  const email = authUser?.email || fallbackEmail;
  if (!email) return;

  const providerName = getFriendlyProfileName(authUser?.displayName, email);
  const pendingName = getFriendlyProfileName(consumePendingSignupName(email), email);
  const fallbackName = email ? email.split('@')[0] : 'Atleta';

  StorageService.login(email);

  const before = StorageService.loadUserProfile?.() || {};
  const isDifferentCloudAccount = Boolean(
    before.cloudUserId &&
    authUser?.id &&
    before.cloudUserId !== authUser.id
  );
  ...
  const current = isDifferentCloudAccount ? {} : (StorageService.loadUserProfile?.() || {});
  const currentName = getFriendlyProfileName(current.displayName, email);
  const displayName = !isGenericProfileName(currentName, email)
    ? currentName
    : (providerName || pendingName || fallbackName);

  const shouldReplacePhoto =
    Boolean(authUser?.avatarUrl) &&
    (!current.photo || current.authProvider === 'supabase' || current.authProvider === 'google' || current.authProvider === 'email');
  ...
  StorageService.saveUserProfile?.({
    ...
    avatar: getProfileInitialFromName(displayName),
    email,
    authProvider: authUser?.provider || current.authProvider || 'supabase',
    cloudUserId: authUser?.id || current.cloudUserId || '',
    photo: shouldReplacePhoto ? authUser.avatarUrl : (current.photo || ''),
    googleAvatarUrl: authUser?.provider === 'google' ? (authUser.avatarUrl || current.googleAvatarUrl || '') : (current.googleAvatarUrl || ''),
    updatedAt: new Date().toISOString()
  });

  updateHeaderUser?.();

  window.RunEvoCloudDataService?.syncProfile?.(StorageService.loadUserProfile?.() || {})
    .catch(error => console.warn('RunEvo: perfil não sincronizado na nuvem.', error));
}

async function finishCloudLogin(authResult, fallbackEmail = '') {
  if (authResult?.needsEmailConfirmation) {
    showEmailConfirmationState(fallbackEmail);
    return;
  }

  const authUser = authResult?.user || await RunEvoCloudAuthService.getCurrentUser?.();

  if (!authUser) {
    showSimpleModal('⚠️', 'Login pendente', 'Não foi possível confirmar a sessão. Tente entrar novamente.');
    return;
  }

  applyAuthenticatedUser(authUser, fallbackEmail);
  finishLogin(authUser.email || fallbackEmail);
}


async function handleCloudAuthBootstrap() {
  if (!window.RunEvoCloudAuthService?.isConfigured?.()) {
    return null;
  }

  try {
    const redirectResult = await RunEvoCloudAuthService.handleAuthRedirectIfNeeded?.();
    ...
    if (redirectResult?.handled) {
      if (redirectResult.error) {
        showSimpleModal(
          '⚠️',
          'Login não concluído',
          translateSupabaseAuthError(redirectResult.errorDescription || redirectResult.error)
        );
        return null;
      }

      if (redirectResult.user?.email) {
        applyAuthenticatedUser(redirectResult.user, redirectResult.user.email);
        showToast('Login confirmado com sucesso.', 'success');
        return redirectResult.user;
      }
    }

    const authUser = await RunEvoCloudAuthService.getCurrentUser?.();

    if (authUser?.email) {
      applyAuthenticatedUser(authUser, authUser.email);
      return authUser;
    }
  } catch (error) {
    console.warn('RunEvo: bootstrap de autenticação cloud falhou.', error);
  }

  return null;
}

function translateSupabaseAuthError(message = '') {
  const text = String(message || '').toLowerCase();

  if (text.includes('provider') || text.includes('oauth')) {
    return 'O provedor de login ainda não está configurado corretamente no Supabase.';
  }

  if (text.includes('redirect') || text.includes('not allowed')) {
    return 'A URL de retorno não está autorizada no Supabase. Confira Site URL e Redirect URLs.';
  }

  if (text.includes('expired') || text.includes('invalid')) {
    return 'O link expirou ou já foi usado. Solicite um novo link e tente novamente.';
  }

  return message || 'Não foi possível concluir o login. Verifique as configurações de autenticação.';
}
...
function setAuthMode(mode = 'login') {
  ...
  if (password) {
    ...
    password.placeholder = isRegister ? 'Crie uma senha' : 'Sua senha';
  }
...
function setAuthError(message = '') {
  ...
  errorEl.textContent = message || 'Não foi possível autenticar agora.';
  errorEl.classList.remove('hidden');
}

function setAuthStatus(type = 'info', title = '', message = '') {
  const statusEl = document.getElementById('auth-status');
  const errorEl = document.getElementById('login-error');
  ...
  if (!statusEl) {
    ...
    if (text) showToast?.(text, type === 'success' ? 'success' : 'info');
    return;
  }

  statusEl.className = `auth-status ${type || 'info'}`;
  statusEl.innerHTML = `
...
function showEmailConfirmationState(email = '') {
  ...
  setAuthStatus(
    ...
    `Enviamos o link de confirmação para ${email}. Depois de confirmar, volte ao RunEvo e faça login.`
  );

  showSimpleModal(
    '📩',
    'Confirme seu e-mail',
    `Enviamos um link de confirmação para ${email}. Confira a caixa de entrada e também o spam.`
  );
}
...
function getReadableAuthError(error) {
  ...
  if (
    ...
    return {
      ...
      message: `Por segurança, o Supabase pediu para aguardar ${seconds} segundos antes de tentar novamente.`,
      inline: `Aguarde ${seconds} segundos para tentar novamente.`
    };
  }
  ...
  if (raw.includes('already registered') || raw.includes('already been registered') || raw.includes('user already registered')) {
    return {
      ...
      message: 'Esse e-mail já está cadastrado. Use Entrar ou recupere sua senha.',
      inline: 'Esse e-mail já está cadastrado. Use Entrar ou Esqueci minha senha.'
    };
  }
  ...
  if (raw.includes('invalid login credentials')) {
    return {
      ...
      inline: 'E-mail ou senha inválidos.'
    };
  }
  ...
  if (raw.includes('email not confirmed')) {
    return {
      ...
      message: 'Sua conta foi criada, mas o e-mail ainda não foi confirmado.',
      inline: 'Confirme seu e-mail antes de entrar.'
    };
  }
  ...
  if (raw.includes('password') && raw.includes('weak')) {
    return {
      title: 'Senha fraca',
      message: 'Use uma senha maior e mais segura.',
      inline: 'Use uma senha maior e mais segura.'
    };
  }
  ...
  return {
    ...
    inline: error?.message || 'Não foi possível autenticar agora.'
  };
}

function startAuthCooldown(seconds = 30) {
  const submitBtn = document.getElementById('auth-submit');
  if (!submitBtn || submitBtn.dataset.cooldown === '1') return;

  let remaining = Math.max(1, Number(seconds) || 30);
  const originalMode = currentAuthMode;
  submitBtn.dataset.cooldown = '1';
  submitBtn.disabled = true;

  const tick = () => {
    submitBtn.textContent = `Tente novamente em ${remaining}s`;
    remaining -= 1;

    if (remaining < 0) {
      submitBtn.dataset.cooldown = '';
...
function getAuthFormData() {
  return {
    ...
    email: UserProfileService.normalizeUsername(document.getElementById('login-username')?.value || ''),
...
async function handleAuthSubmit(event) {
  ...
  const { name, email, password } = getAuthFormData();

  if (authLoginStep === 'reset') {
    await finishPasswordResetFlow();
    return;
  }

  if (currentAuthMode === 'login' && authLoginStep === 'forgot') {
    if (!UserProfileService.isValidEmail(email)) {
  ...
  if (currentAuthMode === 'login' && authLoginStep === 'email') {
    continueAuthEmailStep();
    return;
  }
  ...
  if (currentAuthMode === 'register' && !String(name || '').trim()) {
    setAuthError('Informe o nome do atleta.');
    return;
  }
  ...
  try {
    if (submitBtn) {
      submitBtn.disabled = true;
      submitBtn.textContent = currentAuthMode === 'register' ? 'Criando conta...' : 'Entrando...';
    }

    if (window.RunEvoCloudAuthService?.isConfigured?.()) {
      if (currentAuthMode === 'register') {
        savePendingSignupName(email, name);
        await RunEvoCloudAuthService.signUpWithEmail(email, password, { displayName: name });
        showEmailConfirmationState(email);
        return;
      }

      const result = await RunEvoCloudAuthService.signInWithEmail(email, password);
      await finishCloudLogin(result, email);
      return;
    }

    if (currentAuthMode === 'register') {
      const result = UserProfileService.registerLocalAccount({ email, password, displayName: name });
      if (!result.ok) {
    ...
    if (!UserProfileService.validateCredentials(email, password)) {
    ...
    setAuthError(readable.inline);
    if (readable.title === 'Aguarde um instante' || readable.title === 'Limite de e-mails atingido') {
      const raw = String(error?.message || '').toLowerCase();
      const wait = Number(raw.match(/after\s+(\d+)\s+seconds?/i)?.[1] || 30);
      startAuthCooldown(wait);
      showSimpleModal('⏱️', readable.title, readable.message);
    }
  } finally {
    if (submitBtn) {
      submitBtn.disabled = false;
...
async function handleSocialAuth(provider) {
  ...
  try {
    if (window.RunEvoCloudAuthService?.isConfigured?.()) {
      if (provider === 'apple') await RunEvoCloudAuthService.signInWithApple();
      else await RunEvoCloudAuthService.signInWithGoogle();

      showToast(`Redirecionando para ${label}...`, 'info');
      return;
    }

    showSimpleModal(
      provider === 'apple' ? '' : 'G',
      `${label} Login em preparação`,
      `Configure o Supabase OAuth para ativar login real com ${label}. Enquanto isso, use cadastro por e-mail para testes.`
    );
  } catch (error) {
    console.error(`${label} auth error:`, error);
    const readable = getReadableAuthError(error);
    showSimpleModal('⚠️', `${label} indisponível`, readable.message || `Não foi possível iniciar login com ${label} agora.`);
  }
}

async function showForgotPasswordInfo() {
...
async function sendPasswordRecoveryEmail(email) {
  if (!window.RunEvoCloudAuthService?.isConfigured?.()) {
    showSimpleModal(
      ...
      'A recuperação por e-mail será ativada quando o Supabase estiver configurado.'
    );
    return false;
  }

  try {
    await RunEvoCloudAuthService.sendPasswordReset(email);
    setAuthStatus('success', 'Verifique seu e-mail', 'Enviamos um link para você redefinir sua senha.');
    showSimpleModal('📩', 'E-mail enviado', 'Abra o link recebido para cadastrar uma nova senha.');
    return true;
  } catch (error) {
    console.error('Password reset error:', error);
    const readable = getReadableAuthError(error);
    setAuthError(readable.inline || 'Não foi possível enviar o link agora.');
    if (readable.title === 'Aguarde um instante') {
      const raw = String(error?.message || '').toLowerCase();
      const wait = Number(raw.match(/after\s+(\d+)\s+seconds?/i)?.[1] || 30);
      startAuthCooldown(wait);
      showSimpleModal('⏱️', readable.title, readable.message);
...
async function finishPasswordResetFlow() {
  ...
  try {
    await window.RunEvoCloudAuthService?.updatePassword?.(password);
    ...
    authLoginStep = 'email';
    setAuthMode('login');
    showSimpleModal('✅', 'Senha redefinida', 'Sua senha foi atualizada com sucesso. Faça login para continuar.');
    window.history?.replaceState?.({}, document.title, `${window.location.origin}${window.location.pathname}`);
  } catch (error) {
    console.error('Password update error:', error);
    const readable = getReadableAuthError(error);
    setAuthError(readable.inline || 'Não foi possível redefinir a senha agora.');
  }
}
...
window.addEventListener('load', async () => {
  ...
  setTimeout(async () => {
    ...
    await handleCloudAuthBootstrap();
    if (isPasswordRecoveryUrl()) showResetPasswordScreen();

    if (window.RunEvoCloudAuthService?.isConfigured?.() && !window.__runevoAuthListenerAttached) {
      window.__runevoAuthListenerAttached = true;
      RunEvoCloudAuthService.onAuthStateChange?.((event, session, authUser) => {
        if (authUser?.email && ['SIGNED_IN', 'TOKEN_REFRESHED', 'USER_UPDATED'].includes(event)) {
          applyAuthenticatedUser(authUser, authUser.email);
        }
      });
    }
...
// Navegação principal protegida contra cache antigo.
window.addEventListener('DOMContentLoaded', () => {
  if (window.RunEvoNavigationBound) return;
  window.RunEvoNavigationBound = true;
